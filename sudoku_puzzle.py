    ...
    def is_solved(self):
        ...
        return ("*" not in symbols and
                all([(self._row_set(i) == self._symbol_set and
                      ...
                      self._subsquare_set(i) == self._symbol_set)
                     for i in range(n ** 2)]))
    ...
    def fail_fast(self):
        ...
        for i in range(len(self._symbols)):
            if symbols[i] == "*":
                index.append(i)
        for m in index:
            result = (symbol_set - self._row_set(m)).\
                intersection((symbol_set - self._column_set(m)).
                             intersection(symbol_set - self._subsquare_set(m)))
            if result == set():
                return True
        return False
